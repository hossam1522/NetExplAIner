...
configure_logger(
    name="dataset", filepath=Path(__file__).parent / "data/evaluation/netexplainer.log"
)
...
class Dataset:
    def __init__(self, file_path: str, questions_path: str, windows_context_size: str):
        ...
        if not os.path.exists(file_path):
            ...
            not file_path.endswith(".pcap")
            and not file_path.endswith(".pcapng")
            and not file_path.endswith(".cap")
        ):
            logger.error(
                f"The file {file_path} is not a network file, please provide a pcap or pcapng file"
            )
            raise TypeError(
                f"The file {file_path} is not a network file, please provide a pcap or pcapng file"
            )
        ...
        if not os.path.exists(questions_path):
            ...
            logger.error(
                f"The path {questions_path} is not a file, please provide a file"
            )
            raise FileExistsError(
                f"The path {questions_path} is not a file, please provide a file"
            )
            ...
            logger.error(
                f"The file {questions_path} is not a yaml file, please provide a yaml file"
            )
            raise TypeError(
                f"The file {questions_path} is not a yaml file, please provide a yaml file"
            )
        ...
        for question in self.questions_subquestions.keys():
            if question == "What is the total number of packets in the trace?":
                ...
                if ip_count:
                    ...
                    most_common_ips = [
                        ip for ip, count in ip_count.items() if count == max_count
                    ]
                    most_common_ip = (
                        " or ".join(most_common_ips)
                        if len(most_common_ips) > 1
                        else most_common_ips[0]
                    )
                else:
                ...
                average_packets_per_second = (
                    ...
                    if duration > 0
                    else "There is only one packet in the trace, operation not possible"
                )
